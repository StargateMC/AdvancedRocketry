...
public class RenderStarUIEntity extends Render<EntityUIStar> implements IRenderFactory<EntityUIStar> {
	...
	public void doRender(EntityUIStar entity, double x, double y, double z,
		...
		if(hitObj != null && hitObj.entityHit == entity) {
			...
			RenderHelper.renderTag(Minecraft.getMinecraft().player.getDistanceSq(hitObj.hitVec.z, hitObj.hitVec.y, hitObj.hitVec.x), "ETA: " + (TileWarpShipMonitor.getEnglishTimeFromMs(((long)body.stellarDistanceFrom(entity.world, entity.getPosition()) * 5000))), 0, .15, 0, 5);
