...
public class RenderPlanetUIEntity extends Render<EntityUIPlanet> implements IRenderFactory<EntityUIPlanet> {
	...
	public void doRender(EntityUIPlanet entity, double x, double y, double z,
		...
		if(hitObj != null && hitObj.entityHit == entity) {
			...
			RenderHelper.renderTag(Minecraft.getMinecraft().player.getDistanceSq(hitObj.hitVec.z, hitObj.hitVec.y, hitObj.hitVec.x), "ETA: " + (TileWarpShipMonitor.getEnglishTimeFromMs(((long)properties.stellarDistanceFrom(entity.world, entity.getPosition()) * 5000))), 0, .3, 0, 5);
